...
function _worldMapCoordinates(html, DOM, d3geo, graticule, land, countries, input) {
...
function _textarea(input, html) {
  return function textarea(config = {}) {
    ...
    const form = input({
      form: html`<form>
        <textarea style="display: block; font-size: 0.8em;" name="input">${value}</textarea>
      </form>`,
      title,
      description,
      submit,
...
async function _world() {
  return (await fetch('https://cdn.jsdelivr.net/npm/world-atlas@1/world/110m.json')).json()
}
...
async function _usa() {
  return (await fetch('https://cdn.jsdelivr.net/npm/us-atlas@^2.1/us/states-10m.json')).json()
}
...
export default function define(runtime, observer) {
  ...
  main
    ...
    .define('worldMap1', ['Generators', 'viewof worldMap1'], (G, _) => G.input(_))
