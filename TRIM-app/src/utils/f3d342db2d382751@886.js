...
function _2(md) {
  return md`
In the spirit of _show don’t tell_, below is a scatterplot of the height and weight of Olympic athletes (sourced from [Matt Riggott](https://flother.is/2017/olympic-games-data/)), constructed using a [dot mark](/@observablehq/plot-dot?collection=@observablehq/plot). We assign columns of data (such as _weight_) to visual properties (such as the dot’s _x_), and Plot infers the rest. You can configure much more, if needed, but Plot’s goal is to help you get a meaningful visualization quickly.
  `
}

function _athletes(FileAttachment) {
...
function _dotplot(Plot, athletes) {
  return Plot.dot(athletes, { x: 'weight', y: 'height', stroke: 'sex' }).plot()
}

function _6(Inputs, athletes) {
  return Inputs.table(athletes)
}

function _7(md) {
  return md`
This scatterplot suffers from overplotting: many dots are drawn in the same spot, so it’s hard to perceive density. We can fix this by [binning](/@observablehq/plot-bin?collection=@observablehq/plot) athletes of similar height and weight (and sex), then using opacity to encode the number of athletes in the bin.
  `
}

function _8(Plot, athletes) {
  return Plot.rect(
    athletes,
...
function _10(Plot, athletes) {
  return Plot.density(athletes, { x: 'weight', y: 'height', stroke: 'sex', bandwidth: 6 }).plot()
}

function _11(md) {
  return md`
A simpler take on this data is to focus on one dimension: weight. We can use the bin transform again to make a histogram with weight on the _x_-axis and frequency on the _y_-axis. This plot uses a [rect mark](/@observablehq/plot-rect?collection=@observablehq/plot) and an implicit [stack transform](/@observablehq/plot-stack?collection=@observablehq/plot).
  `
}

function _12(Plot, athletes) {
  return Plot.rectY(athletes, Plot.binX({ y: 'count' }, { x: 'weight', fill: 'sex' })).plot()
}

function _13(md) {
  return md`
Or if we’d prefer to show the two distributions separately as small multiples, we can [facet](/@observablehq/plot-facets?collection=@observablehq/plot) the data along _y_ (keeping the _fill_ encoding for consistency, and adding grid lines and a rule at _y_ = 0 to improve readability).
  `
}

function _14(Plot, athletes) {
  return Plot.plot({
    grid: true,
    marks: [
...
function _17(md) {
  return md`
To learn more, start with [Plot: Marks and Channels](/@observablehq/plot-marks?collection=@observablehq/plot) or dive into the docs below.
...
function _19(htl, previews) {
  return htl.html`${previews(
    [
      {
...
function _previews(htl, preview) {
  return function previews(notebooks, options) {
...
function _preview(htl) {
  return function preview({ path, title, author, thumbnail }, { target = '_blank' } = {}) {
