...
function _gain(Inputs) {
  return Inputs.range([0, 11], { value: 5, step: 0.1, label: 'Gain' })
}

function _now(md) {
  return md`
Now you can reference the input’s value (here _gain_) in any cell, and the cell will run whenever the input changes. No event listeners required!
  `
}

function _gain2(gain) {
  return gain
}

function _currentgain(gain, md) {
  return md`The current gain is ${gain.toFixed(1)}!`
}

function _7(md) {
  return md`
Observable Inputs are released under the [ISC license](https://github.com/observablehq/inputs/blob/main/LICENSE) and depend only on [Hypertext Literal](/@observablehq/htl), our tagged template literal for safely generating dynamic HTML. If you are interested in contributing or wish to report an issue, please see [our repository](https://github.com/observablehq/inputs). For recent changes, please see [our release notes](https://github.com/observablehq/inputs/releases).
  `
}
...
function _n(Inputs) {
  return Inputs.range([0, 255], { step: 1, label: 'Favorite number' })
}
...
function _homeState(Inputs, stateNames) {
  return Inputs.select([null].concat(stateNames), { label: 'Home state' })
}

function _26(homeState) {
  return homeState
}

function _visitedStates(Inputs, stateNames) {
  return Inputs.select(stateNames, { label: 'Visited states', multiple: true })
}

function _28(visitedStates) {
...
function _birthday(Inputs) {
  return Inputs.date({ label: 'Birthday' })
}

function _37(birthday) {
  return birthday
}
...
function _search(Inputs, capitals) {
  return Inputs.search(capitals, { placeholder: 'Search U.S. capitals' })
}
...
function _52(md) {
  return md`
We are grateful to Jeremy Ashkenas for blazing the trail with [“The Grand Native Inputs Bazaar”](/@jashkenas/inputs). To migrate from Jeremy’s inputs to our new official inputs, consider our [legacy inputs](/@observablehq/legacy-inputs).
  `
}
...
function _capitals(FileAttachment) {
  return FileAttachment('us-state-capitals.tsv').tsv({ typed: true })
}

function _stateNames(capitals) {
  return capitals.map((d) => d.State)
}

function _59(md) {
  return md`
The cells below provide deprecated aliases for older versions of Inputs. Please use Inputs.button _etc._ instead of importing from this notebook.
  `
}

function _Button(Inputs) {
  return Inputs.button
}

function _Toggle(Inputs) {
  return Inputs.toggle
}

function _Radio(Inputs) {
  return Inputs.radio
}

function _Checkbox(Inputs) {
  return Inputs.checkbox
}

function _Range(Inputs) {
  return Inputs.range
}

function _Select(Inputs) {
...
function _Textarea(Inputs) {
  return Inputs.textarea
}

function _Search(Inputs) {
...
function _bind(Inputs) {
  return Inputs.bind
}

function _disposal(Inputs) {
  return Inputs.disposal
}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      'us-state-capitals.tsv',
      {
        url: new URL(
          './files/eee4d2a928a36026dab4281b76deb7fe0cd885f1c6df6e546efb79db7e5fa5ccc98a395f865d31c1db3344ff1734683764bb1a63871fb1b39f77bb810f49699c.tsv',
          import.meta.url
        ),
  ...
  main
    .variable(observer('gain'))
    .define('gain', ['Generators', 'viewof gain'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('clicks'))
    .define('clicks', ['Generators', 'viewof clicks'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('mute'))
    .define('mute', ['Generators', 'viewof mute'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('flavors'))
    .define('flavors', ['Generators', 'viewof flavors'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('flavor'))
    .define('flavor', ['Generators', 'viewof flavor'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('viewof homeState'))
    .define('viewof homeState', ['Inputs', 'stateNames'], _homeState)
  main
    .variable(observer('homeState'))
    .define('homeState', ['Generators', 'viewof homeState'], (G, _) => G.input(_))
  main.variable(observer()).define(['homeState'], _26)
  main
    .variable(observer('viewof visitedStates'))
    .define('viewof visitedStates', ['Inputs', 'stateNames'], _visitedStates)
  main
    .variable(observer('visitedStates'))
    .define('visitedStates', ['Generators', 'viewof visitedStates'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('birthday'))
    .define('birthday', ['Generators', 'viewof birthday'], (G, _) => G.input(_))
  ...
  main
    ...
    .define('color', ['Generators', 'viewof color'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('rows'))
    .define('rows', ['Generators', 'viewof rows'], (G, _) => G.input(_))
