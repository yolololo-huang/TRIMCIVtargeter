...
function _vegaVersion() {
  return '5.23.0'
}

function _tooltipVersion() {
  return '0.30.0'
}

function _9(md) {
  return md`
To use the same setup in your own notebooks, add a cell with the following code:
...
function _13(vl, width) {
  ...
  const plot2 = vl
    ...
    .encode(
      vl
        .color()
        .if(click, vl.color().fieldN('weather'))
        .value('lightgray')
        .scale(scale)
        .title('Weather'),
      vl.x().count(),
...
function _population_pyramid(md) {
  return md`
<hr/>
## Population Pyramid
...
function _bind_external(md) {
  return md`
### Bind Parameters to External Inputs
...
function _year(Range) {
  return Range([1850, 2000], { step: 10, value: 2000 })
}
...
function _21(vl) {
  const isOrigin = vl.selectPoint('isOrigin').fields('Origin').bind('legend') // bind to legend interactions
...
function _23(vl, width) {
  const domain = ['Beak Length (mm)', 'Beak Depth (mm)', 'Flipper Length (mm)', 'Body Mass (g)']
...
function _standalone_use(md, apiVersion) {
  return md`<hr/>
...
export default function define(runtime, observer) {
  ...
  main
    .variable(observer('vl'))
    .define('vl', ['vegaVersion', 'vlVersion', 'apiVersion', 'tooltipVersion', 'require'], _vl)
  ...
  main
    .variable(observer('interactive_weather'))
    .define('interactive_weather', ['md'], _interactive_weather)
  main.variable(observer()).define(['vl', 'width'], _13)
  main
    .variable(observer('population_pyramid'))
    .define('population_pyramid', ['md'], _population_pyramid)
  ...
  main
    .variable(observer('year'))
    .define('year', ['Generators', 'viewof year'], (G, _) => G.input(_))
  ...
  main
    .variable(observer('parallel_coordinats'))
    .define('parallel_coordinats', ['md'], _parallel_coordinats)
  main.variable(observer()).define(['vl', 'width'], _23)
  main
    .variable(observer('standalone_use'))
    .define('standalone_use', ['md', 'apiVersion'], _standalone_use)
  return main
}
